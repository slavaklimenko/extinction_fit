...
if __name__ == '__main__':
    ...
    with open('./results/fit_results_sdss_MW.txt', 'w') as file:
        line = ''
        for p in par_names:
                line += ', ' + p + ',' + p+'_err'
    ...
    for Q in s.values():
        ...
        from fitting_model import pars,init_pars,init_MW_template
        ...
        def fit_quasar_spectrum(theta=theta, pars=pars, Q=Q, scale_phot_to_spec_res=10,
                                mask_sdds=False,
                                mask_ir=False, debug=False, photometry=False, return_data=False):
            for el, val in zip([p for p in pars.values() if p.var], theta):
                el.val = val
            #for el in pars.values():
            #    print(el.name,el.val)
            ...
            if pars['Rv'].var == 1:
                rv_scaling = (1/pars['Rv'].val-1/3.1)
                from gordon_extinction import a_coeff,b_coeff
                pars['c1'].val = a_coeff['C1AV'] + b_coeff['C1AV']*rv_scaling
                pars['c2'].val = a_coeff['C2AV'] + b_coeff['C2AV']*rv_scaling
                pars['c3'].val = a_coeff['C3AV'] + b_coeff['C3AV']*rv_scaling
                pars['c4'].val = a_coeff['C4AV'] + b_coeff['C4AV']*rv_scaling
                pars['E0'].val = a_coeff['E0'] + b_coeff['E0']*rv_scaling
                pars['E3'].val = a_coeff['E3'] + b_coeff['E3'] * rv_scaling
                pars['E4'].val = a_coeff['E4'] + b_coeff['E4'] * rv_scaling
                pars['b'].val = a_coeff['g1'] + b_coeff['g1'] * rv_scaling
                pars['alpha'].val = a_coeff['alpha1'] + b_coeff['alpha1'] * rv_scaling
            ...
            if debug:
                ...
                ax[0].plot(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), ext_model/pars['Av'].val, label='ext_model', color='red')
                ax[0].plot(fitting_data.x, -1.086 * np.log(fitting_data.y * scale_spec / fit_composite(fitting_data.x))/pars['Av'].val,
                           color='black',
                           zorder=-100, lw=1)
                ...
                ax[0].fill_between(x=s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs), y1=ext_model /pars['Av'].val,
                                   y2=ext_model_no_bump / pars['Av'].val, color='purple', zorder=-100,
                                   alpha=0.2)
                    ...
                    for el in Q.photo_data.values():
                        if 'SDSS' not in el.name:
                            ...
                            ax[1].errorbar(x=el.l / (1 + Q.z_abs), y=el.f * f_corr / fitting_data.flux_norm,
                                           ...
                                           yerr=el.err * f_corr / fitting_data.flux_norm, fmt='s',lolims=lolims,
                            ...
                            ax[0].errorbar(x=el.l / (1 + Q.z_abs),
                                           y=-1.086 * np.log(el.f * f_corr / fitting_data.flux_norm / model_flux)/pars['Av'].val,
                                           fmt='s', markerfacecolor=color,
                                           xerr=[[(el.l - el.lmin) / (1 + Q.z_abs)],
                                                 [(el.lmax - el.l) / (1 + Q.z_abs)]],
                                           yerr=-1.086 * np.log(el.f / (el.f + el.err))/pars['Av'].val,uplims=uplims,
                                           markeredgecolor='black', ecolor=color)
                            if phot_flux_max < -1.086 * np.log(el.f * f_corr / fitting_data.flux_norm / model_flux)/pars['Av'].val:
                                phot_flux_max = -1.086 * np.log(el.f * f_corr / fitting_data.flux_norm / model_flux)/pars['Av'].val
                ...
                for axs in ax[:]:
                    ...
                    axs.fill_betweenx(y=[-5, 400], x1=0.5, x2=0.6, color='orange', alpha=0.2)
                    axs.fill_betweenx(y=[-5, 400], x1=2.1, x2=2.3, color='orange', alpha=0.2)
                ...
                f_3 = np.nanmean(fit_model[np.abs(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs) - 0.3) < 0.1])
                a_3 = (np.nanmean(ext_model[np.abs(s_composite.x * (1 + Q.z_qso) / (1 + Q.z_abs) - 0.3) < 0.1]))
                ax_0_max = np.max([phot_flux_max,  2.2*a_3/pars['Av'].val])
                ax[0].set_ylim(-0.5*a_3/pars['Av'].val, ax_0_max)
                ...
                fig.savefig('images/'+Q.name+'_ext_fit_sdss.pdf', bbox_inches='tight')
                plt.close()
        ...
        if run_mcmc and 1:
            ...
            if 1:
                with open('./results/mcmc_'+Q.name+'_sdss.pkl', 'wb') as f:
                    pickle.dump((samples, lnprob), f)
        #calc stats mcmc

        with open('./results/mcmc_'+Q.name+'_sdss.pkl', 'rb') as f:
        ...
        if 1:
            with open('./results/fit_results_sdss_MW.txt','a') as file:
                line = ''
                for p in par_names:
        ...
        if 0:
            ...
            with open('./results/extinction_results_sdss.txt', 'a') as file:
                line = ''
                for p in par_names2:
